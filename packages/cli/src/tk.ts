...
const writers = new Set(["init", "create", "update", "close", "reopen", "defer", "undefer", "comment", "dep", "label", "set-state", "import"]);
...
const readInput = async (): Promise<string> => { const chunks: Buffer[] = []; for await (const chunk of stdin) chunks.push(Buffer.from(chunk)); return Buffer.concat(chunks).toString().trimEnd(); };
...
async function rootFrom(from: string): Promise<string | null> { for (let path = resolve(from); ; path = dirname(path)) { try { await access(join(path, ".tasks")); return path; } catch { try { await access(join(path, ".beads")); return path; } catch { if (dirname(path) === path) return null; } } } }
function dataDir(workspace: string): string { try { const fs = require('node:fs'); fs.accessSync(join(workspace, '.tasks')); return join(workspace, '.tasks'); } catch { return join(workspace, '.beads'); } }
async function migrateWorkspace(workspace: string): Promise<boolean> { const oldDir = join(workspace, '.beads'); const newDir = join(workspace, '.tasks'); try { await access(oldDir); } catch { return false; } try { await access(newDir); return false; } catch { /* proceed */ } const { rename: mv } = await import('node:fs/promises'); await mv(oldDir, newDir); const oldDb = join(newDir, 'beads.db'); const newDb = join(newDir, 'tasks.db'); try { await access(oldDb); await mv(oldDb, newDb); } catch { /* no db to rename */ } return true; }
...
class CommandService {
  ...
  async importJsonl(input: string): Promise<number> {
    const rows = input.split(/\r?\n/).filter(Boolean).map((line, index) => {
      try { return JSON.parse(line) as unknown; } catch { fail(`import line ${index + 1} field $: invalid JSONL`); }
    });
    const imported: Issue[] = rows.map((row, index) => {
      try { return issueFromBdWire(row, canonicalTimestampCodec).issue; }
      catch (cause) {
        const error = cause as { issues?: readonly { path: readonly (string | number)[]; message: string }[]; message?: string };
        const detail = error.issues?.[0];
        const field = detail?.path.length ? detail.path.join(".") : "$";
        fail(`import line ${index + 1} field ${field}: ${detail?.message ?? error.message ?? "invalid record"}`);
        return fail("unreachable");
      }
    });
    return transaction(this.database, async (uow) => { for (const issue of imported) unwrap(await uow.save(issue)); return imported.length; });
  }
